          ...
          {scriptSteps.length > 0 ? (
            <>
              ...
              {activeReplayTab === "steps" && (
                <>
                  {availableColumns.length > 0 && (
                    <div className="mb-3 flex items-center gap-2 rounded-lg border border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50/60 px-3 py-2.5">
                      <Database className="size-3.5 text-amber-500 shrink-0" />
                      {selectedDatasetRowIndex !== null ? (
                        <>
                          <div className="flex size-4 shrink-0 items-center justify-center rounded-full bg-amber-500 text-[9px] font-bold text-white">
                          ...
                          <button
                            ...
                            className="ml-auto text-[10px] text-amber-400 hover:text-amber-700 transition-colors"
                          >
                            Clear
                          </button>
                        </>
                ...
                <>
                  ...
                  {(() => {
                    ...
                    const steps3 = [
                      ...
                      { label: "Run all", done: batchDone || busyAction === "batch" },
                    ];
                    return (
                      <div className="mb-4 flex items-center gap-2 text-[11px]">
                        {steps3.map(({ label, done }, i) => (
                          <span key={label} className="flex items-center gap-1.5">
                            {i > 0 && (
                              <span className={steps3[i - 1].done ? "text-emerald-300" : "text-slate-200"}>→</span>
                            )}
                            <span className={`flex items-center gap-1 ${done ? "text-emerald-600 font-medium" : "text-slate-400"}`}>
                              {done
                                ? <CheckCircle className="size-3 text-emerald-500" />
                                : <span className="flex size-4 items-center justify-center rounded-full bg-slate-100 text-[10px] font-bold">{i + 1}</span>
                              }
                              {label}
                            </span>
                          </span>
                  ...
                  />
                  {datasetDetail && (
                    <div className="mt-4 rounded-xl border-2 border-sky-300 bg-gradient-to-br from-sky-50 to-indigo-50/40 px-5 py-4 space-y-3 shadow-sm">
                      <div className="flex items-center gap-3">
                        <div className="flex size-8 items-center justify-center rounded-lg bg-sky-100 shadow-inner">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] text-sky-500">One test run will be queued per dataset row</p>
                        </div>
                      </div>
                      ...
                      {confirmBatch ? (
                        <div className="flex items-center gap-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 animate-in fade-in duration-200">
                          <AlertTriangle className="size-4 text-amber-600 shrink-0" />
                          <p className="text-xs text-amber-800 flex-1">
                            This will queue <strong>{datasetDetail.rows?.length ?? 0} test runs</strong>. Continue?
                          </p>
