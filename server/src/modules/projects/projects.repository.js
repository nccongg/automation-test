...
async function getProjectById(userId, projectId) {
  // Ensure projectId is a number for BIGSERIAL comparison
  ...
  const statsSql = `
    WITH stats AS (
      SELECT
        ...
        COUNT(DISTINCT tr.id) FILTER (WHERE tr.verdict IN ('pass', 'fail', 'error', 'partial')) AS total_runs,
        CASE
          WHEN COUNT(DISTINCT tr.id) FILTER (WHERE tr.verdict IN ('pass', 'fail', 'error', 'partial')) = 0 THEN NULL
          ELSE ROUND(
            (COUNT(DISTINCT tr.id) FILTER (WHERE tr.verdict = 'pass')::NUMERIC
            / NULLIF(
              COUNT(DISTINCT tr.id) FILTER (WHERE tr.verdict IN ('pass', 'fail', 'error', 'partial')),
              0
            )::NUMERIC
          ) * 100, 1)
        END AS pass_rate
    ...
    SELECT
      ...
      CASE
        ...
        ELSE 'failing'
      END AS status,
      (SELECT verdict FROM test_runs tr2
